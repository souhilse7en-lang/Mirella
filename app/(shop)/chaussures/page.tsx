...
export default async function ChaussuresPage({
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 lg:px-8 py-16">
      <div className="mb-12">
        <p className="text-xs uppercase tracking-[0.25em] text-muted-foreground mb-2">Sandales · Escarpins · Sneakers</p>
        <h1 className="text-4xl font-semibold tracking-tight mb-2">Chaussures</h1>
      ...
      {products && products.length > 0 ? (
        <>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 mt-16">
              {page > 1 && (
                <Link href={`/chaussures?page=${page - 1}`} className="px-4 py-2 text-sm border border-border rounded-lg hover:bg-muted transition-colors">
                  ← Précédent
                </Link>
              )}
        ...
        <p className="text-muted-foreground text-center py-24">Aucune chaussure disponible pour le moment.</p>
      )}
    </div>
  );
}
